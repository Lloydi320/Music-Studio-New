  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function updateStartDateMinBasedOnClosing() {
        ...
        // Allow selecting today and any future date; no 24-hour lead time
        startDateInput.min = todayStr;
        // Do not auto-adjust the user's selection here; validation handles past dates
      }
      ...
      // Fetch booked dates (instrument only), but do not block future dates
      async function fetchBookedDates() {
        try {
          const instrumentResp = await fetch('/api/instrument-rental/booked-dates');
          const instrumentData = await instrumentResp.json();
          // Keep calendar open: only grey out past days, not future bookings
          bookedDates = [];
      ...
      function validateDateSelection(event) {
        ...
        // Do not block future dates based on existing bookings; only prevent past dates
        input.classList.remove('date-unavailable');
        
        // Range conflict checks removed: instrument rental allows future dates unless past
        return true;
      }
       
       // Booked-dates prefetch disabled; availability checks run on change
       // fetchBookedDates();

      // Update instruments when type changes
      instrumentTypeSelect.addEventListener('change', async function() {
        ...
        // Trigger availability check when type changes
        await checkAvailabilityClient(false);
      });

      // Update rate when specific instrument changes
      instrumentNameSelect.addEventListener('change', async function() {
      ...
      showSummaryBtn.addEventListener('click', async function() {
        // Validate form first
        if (!validateForm()) {
          return;
        }

        // Client-side availability check with backend API
        const ok = await checkAvailabilityClient(true);
        if (!ok) {
          return; // Do not open modal when blocked
        }
      ...
      const availabilityAlertId = 'availability-block-alert';
      function ensureAlertContainer() {
        let el = document.getElementById(availabilityAlertId);
        if (!el) {
          el = document.createElement('div');
          el.id = availabilityAlertId;
          ...
          const parent = startDateGroup || startDateInput.parentNode;
          parent?.parentNode?.insertBefore(el, parent.nextSibling);
        }
        return el;
      }
      function showAvailabilityBlock(message) {
        const el = ensureAlertContainer();
        el.textContent = message || 'Selected date is not available.';
        el.style.display = 'block';
        if (showSummaryBtn) showSummaryBtn.disabled = true;
        startDateInput.classList.add('date-unavailable');
      }
      function hideAvailabilityBlock() {
        const el = document.getElementById(availabilityAlertId);
        if (el) { el.style.display = 'none'; el.textContent = ''; }
        if (showSummaryBtn) showSummaryBtn.disabled = false;
        startDateInput.classList.remove('date-unavailable');
      }

      // Core: client-side availability check using backend API
      async function checkAvailabilityClient(isOnSummaryClick) {
        try {
          hideAvailabilityBlock();
          const instrumentType = instrumentTypeSelect?.value || '';
          const instrumentName = instrumentType === 'full_package' ? 'full_package' : (instrumentNameSelect?.value || '');
          const startDate = startDateInput?.value || '';
          const endDate = endDateInput?.value || startDate;

          if (!instrumentType || !startDate) {
            if (isOnSummaryClick) {
              alert('Please select instrument type and date.');
            }
            return false;
          }
          if (instrumentType !== 'full_package' && !instrumentName) {
            if (isOnSummaryClick) {
              alert('Please select a specific instrument.');
            }
            return false;
          }
          ...
          const res = await fetch('/api/check-rental-availability', {
            method: 'POST',
            headers: {
          ...
          if (!res.ok) {
            showAvailabilityBlock(data?.message || 'Selected date is not available.');
          ...
          showAvailabilityBlock('Unable to check availability. Please try again.');
          return false;
        }
      }
