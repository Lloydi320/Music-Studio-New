...
class InstrumentRentalController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Block other instruments if a Drum Set or Full Package rental overlaps the requested range
            $normalizedTypeForBlock = strtolower(str_replace(' ', '_', $validatedData['instrument_type']));
            $blockedByDrumsOrFull = InstrumentRental::whereIn('status', ['pending', 'confirmed'])
                ->where(function ($q) {
                    $q->where('instrument_type', 'drums')
                      ->orWhere('instrument_type', 'full_package');
                })
                ->where(function ($query) use ($validatedData) {
                    $query->whereBetween('rental_start_date', [$validatedData['rental_start_date'], $validatedData['rental_end_date']])
                          ...
                          ->orWhere(function ($qq) use ($validatedData) {
                              $qq->where('rental_start_date', '<=', $validatedData['rental_start_date'])
                                 ->where('rental_end_date', '>=', $validatedData['rental_end_date']);
                          });
                })
                ->exists();

            if ($blockedByDrumsOrFull && !in_array($normalizedTypeForBlock, ['drums', 'full_package'])) {
                return back()->withErrors(['rental_start_date' => 'Selected dates are blocked due to an existing Drum Set or Full Package rental.'])->withInput();
            }
    ...
    public function checkAvailability(Request $request)
    {
        try {
            ...
            // Block other instruments if Drum Set or Full Package overlaps
            $normalizedType = strtolower(str_replace(' ', '_', $request->instrument_type));
            $blockedByPrimarySets = InstrumentRental::whereIn('status', ['pending', 'confirmed'])
                ->where(function ($q) {
                    $q->where('instrument_type', 'drums')
                      ->orWhere('instrument_type', 'full_package');
                })
                ->where(function ($query) use ($request) {
                    $query->whereBetween('rental_start_date', [$request->rental_start_date, $request->rental_end_date])
                          ...
                          ->orWhere(function ($qq) use ($request) {
                              $qq->where('rental_start_date', '<=', $request->rental_start_date)
                                 ->where('rental_end_date', '>=', $request->rental_end_date);
                          });
                })
                ->exists();

            if ($blockedByPrimarySets && !in_array($normalizedType, ['drums', 'full_package'])) {
                return response()->json([
                    'available' => false,
                    'message' => 'This date range is blocked due to a Drum Set or Full Package rental.'
                ]);
            }

            // Check for conflicting rentals of the same instrument
