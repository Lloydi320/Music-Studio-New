...
class BookingController extends Controller
{
        ...
        try {
            ...
            if ($bookingType === 'studio_rental') {
                ...
                ActivityLog::logActivity(
                    ...
                    ActivityLog::ACTION_RENTAL_UPDATED,
                    $rental->user_id,
    ...
    {
        try {
            // Collect dates that have any pending or confirmed studio bookings
            ...
            // Include dates from instrument rentals that were rescheduled and approved
            // We detect approved reschedules via non-null reschedule_source and confirmed status
            $rescheduledRentalDates = \App\Models\InstrumentRental::where('status', 'confirmed')
                ->whereNotNull('reschedule_source')
                ->get(['rental_start_date', 'rental_end_date'])
                ->flatMap(function ($rental) {
                    $dates = [];
                    $start = \Carbon\Carbon::parse($rental->rental_start_date)->startOfDay();
                    $end = \Carbon\Carbon::parse($rental->rental_end_date)->startOfDay();
                    $cursor = $start->copy();
            ...
            $combined = array_values(array_unique(array_merge($datesWithBookings, $rescheduledRentalDates)));

            return response()->json([
                'booked_dates' => $combined
            ]);
        } catch (\Exception $e) {
